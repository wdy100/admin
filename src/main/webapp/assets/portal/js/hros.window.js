...
/*					$(windowId).find('iframe').on('load', function(){
					...
					$(windowId).find('iframe').on('load', function(){
						try{
							$(windowId).find(".window-frame>iframe").get(0).contentWindow.currentappid = options.appid;
							if(customWindow){
								$(windowId).find(".window-frame>iframe").get(0).contentWindow.customWindow = customWindow;
							}
						}catch (e) {
							// 跨域问题
						}
				...
				nextDo({
					type : type,
					appid : appid,
					imgsrc_val : 'portal/img/ui/default_icon.png',
					imgsrc : Util.fullUrl(HROS.CONFIG.basicStaticUrl,'portal/img/ui/default_icon.png'),
					title : obj.title,
					url : obj.url,
					width : obj.width,
					height : obj.height,
		...
		*/
		create : function(appid){
			...
			if(iswindowopen == false && $('#d_' + appid).attr('opening') != 1){
				...
				function nextDo(options){
					var windowId = '#w_' + options.appid;
					var top = (HROS.window.getDeskHeight() - options.height) / 2 - 20 <= 0 ? 0 : (HROS.window.getDeskHeight() - options.height) / 2 - 10;
					...
					switch(options.type){
						...
						case 'papp':
							...
							HROS.taskbar.resize();
							//新增窗口
							//一票到底台账要求显示为‘HIG销售全流程一票到底台账’
							var reportTitle = options.title
							if(reportTitle =='一票到底台账'){
								reportTitle='HIG销售全流程一票到底台账';
							}
							TEMP.windowTemp = {
								'width' : options.width,
								'height' : options.height,
						...
						case 'folder':
							...
							HROS.CONFIG.createIndexid += 1;
							//载入文件夹内容
							$.getJSON(HROS.CONFIG.basicActionUrl + '/getMyFolderApp.action?folderid=' + options.appid, function(sc){
								if(sc != null){
									var folder_append = '';
									for(var i = 0; i < sc.length; i++){
										folder_append += appbtnTemp({
											...
											'imgsrc' : Util.fullUrl(HROS.CONFIG.basicStaticUrl,sc[i]['icon'])
										});
									}
				...
				$.getJSON(HROS.CONFIG.basicActionUrl + '/getMyAppById.action?id=' + appid, function(app){
					if(app != null){
						if(app['error'] == 'E100'){
							...
							nextDo({
								...
								title : app['name'],
								imgsrc_val:app['icon'],
								imgsrc : Util.fullUrl(HROS.CONFIG.basicStaticUrl,app['icon']),
		...
		max : function(appid){
			...
			$(windowId).addClass('window-maximize').attr('ismax',1).animate({
				width : '100%',
				height : HROS.window.getDeskHeight()-30,
				top : '10px',
				left : 0
			}, 200);
		...
		updateFolder : function(appid){
			HROS.window.show2top(appid);
			var windowId = '#w_' + appid, taskId = '#t_' + appid;
			$.getJSON(HROS.CONFIG.basicActionUrl + '/getMyFolderApp.action?folderid=' + appid, function(sc){
				if(sc != null){
					var folder_append = '';
					for(var i = 0; i < sc.length; i++){
						folder_append += appbtnTemp({
			...
			obj.on('dblclick', '.title-bar', function(e){
				...
				if(obj.attr('realappid') !== 0){
					HROS.window.createTemp({
						appid : 'hoorayos-yysc',
						title : '应用市场',
						url : HROS.CONFIG.basicActionUrl+'/appmarketInit.action?id=' + obj.attr('realappid'),
						width : 800,
						height : 484,
						isflash : false,
						refresh : true
