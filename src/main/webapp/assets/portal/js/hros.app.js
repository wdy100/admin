	...
	return {
		...
		*/
		getXY : function(callback){
			$.ajax({
				type : 'POST',
				url : HROS.CONFIG.basicActionUrl+'/getAppXY.action',
				data : ''
			}).done(function(i){
				HROS.CONFIG.appXY = i;
				callback && callback();
		...
		*/
		updateXY : function(i, callback){
			$.ajax({
				type : 'POST',
				url : HROS.CONFIG.basicActionUrl+'/updateAppXY.action',
				data : 'appxy=' + i
			}).done(function(){
				HROS.CONFIG.appXY = i;
				callback && callback();
		...
		*/
		get : function(){
			...
			//获取json数组并循环输出每个图标
			$.getJSON(HROS.CONFIG.basicActionUrl + '/getMyApp.action', function(sc){
				//加载应用码头图标
				if(sc['dock'] != ''){
					var dock_append = '';
					$(sc['dock']).each(function(i){
						dock_append += appbtnTemp({
							...
							'imgsrc' : Util.fullUrl(HROS.CONFIG.basicStaticUrl,this.icon)
						});
					});
					...
					if(sc['desk' + j] != ''){
						$(sc['desk' + j]).each(function(i){
							...
							if(this.fileId == null || this.fileId == ''){
								tmpImgSrc = Util.fullUrl(HROS.CONFIG.basicStaticUrl,this.icon);
							}else{
								tmpImgSrc = Util.fullUrl(HROS.CONFIG.basicDynamicUrl,'security/download.action?id='+this.fileId);
							}
				...
				$('#desk').off('click').on('click', 'li.add', function(){
					HROS.window.createTemp({
						appid : 'hoorayos-yysc',
						title : '应用市场',
						url : HROS.CONFIG.basicActionUrl+'/appmarketInit.action',
						width : 800,
						height : 520,
						isflash : false
					});
		...
		*/
		add : function(id, callback){
			$.ajax({
				type : 'POST',
				url : HROS.CONFIG.basicActionUrl+'/addMyApp.action',
				data : 'id=' + id  + '&desk=' + HROS.CONFIG.desk,
				success : function(){
					callback && callback();
				}
			}); 
		...
		*/
		remove : function(id, callback){
			$.ajax({
				type : 'POST',
				url : HROS.CONFIG.basicActionUrl+'/delMyApp.action',
				data : 'id=' + id,
				success : function(){
						...
						if(folderId != null){
							if(oldobj.hasClass('folder') == false){
								$.ajax({
									...
									data : 'movetype=dock-folder&id=' + oldobj.attr('appid') + '&to=' + folderId,
									success : function(){
										oldobj.remove();
							...
							var dock_h2 = HROS.CONFIG.dockPos == 'top' ? 0 : 0;
							icon2 = HROS.grid.searchDockAppGrid(cx - dock_w2, cy - dock_h2 - HROS.CONFIG.headerHeight);
							if(icon2 != null && icon2 != oldobj.index()){
								$.ajax({
									type : 'POST',
									url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
									data : 'movetype=dock-dock&id=' + oldobj.attr('appid') + '&from=' + oldobj.index() + '&to=' + icon2,
									success : function(){
										if(icon2 < iconIndex2){
								...
								if(icon != null){
									...
									$.ajax({
										type : 'POST',
										url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
										data : 'movetype=dock-desk&id=' + oldobj.attr('appid') + '&from=' + oldobj.index() + '&to=' + (icon + 1) + '&desk=' + HROS.CONFIG.desk,
										success : function(){
											if(icon < iconIndex){
						...
						if(folderId != null){
							if(oldobj.attr('type') != 'folder'){
								$.ajax({
									type : 'POST',
									url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
									data : 'movetype=desk-folder&id=' + oldobj.attr('appid') + '&from=' + (oldobj.index() - 2) + '&to=' + folderId + '&desk=' + HROS.CONFIG.desk,
									success : function(){
										oldobj.remove();
										HROS.deskTop.appresize();
										//如果文件夹预览面板为显示状态，则进行更新
							...
							if(icon2 != null){
								$.ajax({
									type : 'POST',
									url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
									data : 'movetype=desk-dock&id=' + oldobj.attr('appid') + '&from=' + (oldobj.index() - 2) + '&to=' + (icon2 + 1) + '&desk=' + HROS.CONFIG.desk,
									success : function(){
										if(icon2 < iconIndex2){
											$('#dock-bar .dock-applist li:eq(' + icon2 + ')').before(oldobj);
										}else if(icon2 > iconIndex2){
								...
								if(icon != null && icon != (oldobj.index() - 2)){
									$.ajax({
										type : 'POST',
										url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
										data : 'movetype=desk-desk&id=' + oldobj.attr('appid') + '&from=' + (oldobj.index() - 2) + '&to=' + icon + '&desk=' + HROS.CONFIG.desk,
										success : function(){
											if(icon < iconIndex){
												$('#desk-' + HROS.CONFIG.desk + ' li:not(.add):eq(' + icon + ')').before(oldobj);
											}else if(icon > iconIndex){
						...
						if(folderId != null){
							if(oldobj.parents('.folder-window').attr('appid') != folderId){
								$.ajax({
									type : 'POST',
									url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
									data : 'movetype=folder-folder&id=' + oldobj.attr('appid') + '&to=' + folderId,
									success : function(){
										oldobj.remove();
										HROS.deskTop.appresize();
										//如果文件夹预览面板为显示状态，则进行更新
							...
							if(icon2 != null){
								$.ajax({
									type : 'POST',
									url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
									data : 'movetype=folder-dock&id=' + oldobj.attr('appid') + '&to=' + (icon2 + 1) + '&desk=' + HROS.CONFIG.desk,
									success : function(){
								...
								if(icon != null){
									...
									$.ajax({
										type : 'POST',
										url : HROS.CONFIG.basicActionUrl+'/updateMyApp.action',
										data : 'movetype=folder-desk&id=' + oldobj.attr('appid') + '&to=' + (icon + 1) + '&desk=' + HROS.CONFIG.desk,
										success : function(){
											var folderId = oldobj.parents('.folder-window').attr('appid');
											if(icon < iconIndex){
